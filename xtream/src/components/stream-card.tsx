  ...
  return (
    <Card className="w-full max-w-2xl mx-auto overflow-hidden border-border/60 bg-card shadow-lg rounded-xl">
      ...
      <div className="relative aspect-video">
        ...
        {!stream.live && stream.startTime && new Date(stream.startTime) > new Date() && (
          <div className="absolute top-3 left-3">
            <Badge variant="secondary" className="bg-background/80 text-foreground">
              Starts {formatDistanceToNow(new Date(stream.startTime), { addSuffix: true })}
            </Badge>
          </div>
        )}
      </div>
