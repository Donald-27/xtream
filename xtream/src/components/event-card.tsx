...
export function EventCard({ event }: EventCardProps) {
  ...
  const userDocRef = useMemoFirebase(() => {
    if (!firestore || !event.organizerId) return null;
    return doc(firestore, 'users', event.organizerId);
  }, [firestore, event.organizerId]);

  const { data: userProfile } = useDoc<User>(userDocRef);

  const startTime = event.startTime ? new Date(event.startTime.seconds * 1000) : new Date();
