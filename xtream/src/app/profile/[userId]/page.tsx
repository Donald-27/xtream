...
import { useDoc, useSupabase } from "@/lib/supabase/hooks";
...
export default function UserProfilePage() {
  ...
  const { data: rawUserProfile, isLoading } = useDoc<any>(
    targetUserId ? 'users' : null,
    targetUserId
  );
  ...
  const { data: rawCurrentUserProfile } = useDoc<any>(
    currentUser ? 'users' : null,
    currentUser?.id
  );

  const currentUserProfile = useMemo(() => {
    if (!rawCurrentUserProfile) return null;
    return mapDbUserToUser(rawCurrentUserProfile);
  }, [rawCurrentUserProfile]);
  ...
  const handleFollowToggle = async () => {
    if (!currentUser || !userProfile || !supabase) {
    ...
    const targetFollowerIds = userProfile?.followerIds || [];

    if (isFollowing) {
      const newFollowingIds = currentFollowingIds.filter(id => id !== userProfile.id);
      const newFollowerIds = targetFollowerIds.filter(id => id !== currentUser.id);
      ...
      await supabase
        .from('users')
        .update({ follower_ids: newFollowerIds })
        .eq('id', userProfile.id);

      toast({ title: 'Unfollowed', description: `You are no longer following ${userProfile.username}.` });
    } else {
      const newFollowingIds = [...currentFollowingIds, userProfile.id];
      const newFollowerIds = [...targetFollowerIds, currentUser.id];
