...
import { Plus, Search, RadioTower, CalendarDays, Users, MapPin, WifiOff } from "lucide-react";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Input } from "@/components/ui/input";
import { useCollection } from "@/lib/supabase/hooks";
import { useUser } from "@/lib/supabase/provider";
import { mapDbEventToEvent, mapDbBeaconToBeacon } from "@/lib/types";
...
export default function ConnectPage() {
  ...
  const { data: rawEvents, isLoading: isLoadingEvents } = useCollection<any>({
    table: 'events',
    orderBy: { column: 'start_time', ascending: false },
  });

  const { data: rawBeacons, isLoading: isLoadingBeacons } = useCollection<any>({
    table: 'beacons',
    filters: [{ column: 'expires_at', operator: 'gt', value: now }],
    orderBy: { column: 'expires_at', ascending: true },
  });

  const events = useMemo(() => rawEvents?.map(mapDbEventToEvent) || [], [rawEvents]);
  const beacons = useMemo(() => rawBeacons?.map(mapDbBeaconToBeacon) || [], [rawBeacons]);
  ...
  const handleFindNearby = async () => {
    ...
    try {
      ...
      const nearbyUsers = data.users.filter((u: User) => u.id !== user?.id);
        ...
        <Tabs defaultValue="events" className="w-full">
          ...
          <TabsContent value="nearby">
            <div className="text-center py-10 px-4 rounded-lg bg-card border border-border/60 flex flex-col items-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground max-w-sm">
                Discover other Xtream users on the same Wi-Fi network as you.
              </p>
          ...
          <TabsContent value="beacons">
            <div className="flex flex-col gap-4">
              ...
              {beacons.length === 0 && !isLoadingBeacons && (
